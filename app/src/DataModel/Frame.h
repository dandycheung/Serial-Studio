...
// MD5 of optional Project Editor password (UX read-only flag, not crypto).
...
[[nodiscard]] inline bool read(Source& s, const QJsonObject& obj)
{
  ...
  // Prefer canonical keys; fall back to legacy "checksum"/"decoder" aliases (writer emits both).
...
[[nodiscard]] inline bool read(Dataset& d, const QJsonObject& obj)
{
  ...
  // Legacy single-field "alarm": seed alarmHigh when neither high nor low is present.
  ...
  // Normalize swapped min/max pairs from legacy / hand-edited files (writer already sorts).
