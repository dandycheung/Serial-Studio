  ...
  , m_transformEditor(nullptr)
{
  ...
  connect(
    ...
    [this] {
      ...
      // Fall back to the Groups parent category instead of jumping to the project root.
      if (m_groupsRootItem) {
  ...
  connect(
    ...
    [this](int survivingGroupId) {
      ...
      // Group was emptied and removed: fall back to the Groups parent node, not the project root.
      if (m_groupsRootItem) {
        m_selectionModel->setCurrentIndex(m_groupsRootItem->index(),
                                          QItemSelectionModel::ClearAndSelect);
        return;
...
void DataModel::ProjectEditor::openTransformEditor()
{
  ...
  // Lazy-create on first use to avoid destruction-order crash.
