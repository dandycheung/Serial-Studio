...
void DataModel::LuaScriptEngine::createState()
{
  ...
  // Deadline starts at "never"; set per-call before entering Lua.
...
void DataModel::LuaScriptEngine::watchdogHook(lua_State* L, lua_Debug* ar)
{
  ...
  // Defensive: no engine pointer stored.
  if (!self) [[unlikely]]
    return;
                                            ...
                                            bool showMessageBoxes)
{
  ...
  // Execute the chunk under the watchdog (file-scope `while true do end` is possible).
