...
void Widgets::GPS::updateTiles()
{
  ...
  // Map center in base-zoom tile coordinates.
  ...
  for (int dx = -tilesX / 2; dx <= tilesX / 2; ++dx) {
    for (int dy = -tilesY / 2; dy <= tilesY / 2; ++dy) {
      ...
      // Preload next zoom level when fractional zoom > 0.7 for smooth transitions.
...
void Widgets::GPS::paintMap(QPainter* painter, const QSize& view)
{
  ...
  // Pixel offset rounded to integer to avoid tile seams during auto-tracking.
